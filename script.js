  ...
  const now = new Date();
    ...
    const allColumns = [
        // January
        { month: 'January', date: 10 }, { month: 'January', date: 11 }, { month: 'January', date: 17 },
        { month: 'January', date: 18 }, { month: 'January', date: 24 }, { month: 'January', date: 25 }, { month: 'January', date: 31 },
        // February
        { month: 'February', date: 1 }, { month: 'February', date: 7 }, { month: 'February', date: 8 },
        { month: 'February', date: 14 }, { month: 'February', date: 15 }, 
        { month: 'February', date: 21, exp: { content: '1', span: 2 } }, { month: 'February', date: 22 },
        { month: 'February', date: 28, exp: { content: '2', span: 2 } },
        // March
        { month: 'March', date: 1 }, 
        { month: 'March', date: 7, exp: { content: '3', span: 2 } }, { month: 'March', date: 8 },
        { month: 'March', date: 14 }, { month: 'March', date: 15 }, { month: 'March', date: 21 }, { month: 'March', date: 22 },
        // April Gap
        { month: 'April', isGap: true },
        // May
        { month: 'May', date: 2 }, { month: 'May', date: 3 }, { month: 'May', date: 9 }, { month: 'May', date: 10 },
        { month: 'May', date: 16, exp: { content: '4', span: 2 } }, { month: 'May', date: 17 },
        { month: 'May', date: 23, exp: { content: '5', span: 2 } }, { month: 'May', date: 24 },
        { month: 'May', date: 30, exp: { content: '6', span: 2 } }, { month: 'May', date: 31 },
        // June
        { month: 'June', date: 6, exp: { content: '7', span: 2 } }, { month: 'June', date: 7 },
        { month: 'June', date: 13 }, { month: 'June', date: 14 },
        { month: 'June', date: 20, exp: { content: '8', span: 2 } }, { month: 'June', date: 21 },
        { month: 'June', date: 27 }, { month: 'June', date: 28 },
        // July
        { month: 'July', date: 4, exp: { content: '9', span: 2 } }, { month: 'July', date: 5 },
        { month: 'July', date: 11 }, { month: 'July', date: 12 },
        { month: 'July', date: 18, exp: { content: '<span style="font-size:0.9rem">10</span>', span: 1 } },
        { month: 'July', date: 19, exp: { content: '<span class="vertical-text">အာဇာနည်နေ့</span>', span: 1 } },
        { month: 'July', date: 25 }, { month: 'July', date: 26 },
        // August
        { month: 'August', date: 1, exp: { content: '11', span: 2 } }, { month: 'August', date: 2 },
        { month: 'August', date: 8 }, { month: 'August', date: 9 },
        { month: 'August', date: 15, exp: { content: '12', span: 2 } }, { month: 'August', date: 16 },
        { month: 'August', date: 22, exp: { content: '<span class="makeup-text">Make up</span>', span: 2 } }, { month: 'August', date: 23 },
        { month: 'August', date: 29, exp: { content: '<span class="makeup-text">Make up</span>', span: 2 } }, { month: 'August', date: 30 },
        // September
        { month: 'September', date: 5, exp: { content: '<span class="makeup-text">Make up</span>', span: 2 } }, { month: 'September', date: 6 },
        { month: 'September', date: 12 }, { month: 'September', date: 13 },
        { month: 'September', date: 19 }, { month: 'September', date: 20 },
        { month: 'September', date: 26 }, { month: 'September', date: 27 }
    ];
    ...
    allColumns.forEach(col => {
        ...
        if (last && last.month === col.month) { last.count++; } else { monthGroups.push({ month: col.month, count: 1 }); }
    });

    monthGroups.forEach(group => {
        const th = document.createElement('th');
        th.className = 'month-header' + (group.month === 'April' ? ' april-column' : '');
        ...
        if (skipExp > 0) {
            ...
            if (col.isGap) {
                ...
                for (let i = 0; i < col.exp.span; i++) {
                    ...
                    if (futureCol && futureCol.month === currentMonthName && futureCol.date === currentDay) {
                        expCoversToday = true;
                        break;
                    }
                }
 ...
 // --- WEATHER ICON MAPPING ---
    ...
    function getWeatherDescription(code) {
        const desc = {0:'Clear Sky', 1:'Mainly Clear', 2:'Partly Cloudy', 3:'Overcast', 45:'Foggy', 48:'Foggy', 51:'Drizzle', 53:'Drizzle', 55:'Drizzle', 61:'Rainy', 63:'Rainy', 65:'Heavy Rain', 71:'Snowy', 73:'Snowy', 75:'Heavy Snow', 80:'Rain Showers', 81:'Rain Showers', 82:'Heavy Showers', 95:'Thunderstorm', 96:'Thunderstorm', 99:'Thunderstorm'};
    ...
    async function getW() {
        try {
            ...
            const avgHum = Math.round(d.hourly.relative_humidity_2m.slice(hStart, hStart+24).reduce((a,b)=>a+b)/24);

            document.getElementById('main-weather-icon').textContent = getWeatherIcon(wCode);
            document.getElementById('current-temp').textContent = tCur;
            document.getElementById('weather-condition').textContent = getWeatherDescription(wCode);
            ...
            const metrics = [
                ...
                {label:'Wind', val:Math.round(d.daily.windspeed_10m_max[todayIdx]), unit:'km/h'},
                {label:'UV Index', val:d.daily.uv_index_max[todayIdx], unit:''},
                {label:'Rain Prob', val:d.daily.precipitation_probability_max[todayIdx], unit:'%'},
                {label:'Sunrise', val:d.daily.sunrise[todayIdx].split('T')[1], unit:''},
                {label:'Sunset', val:d.daily.sunset[todayIdx].split('T')[1], unit:''}
            ];
            ...
            // Next 2 Days
            const fBoard = document.getElementById('forecast-2-days'); fBoard.innerHTML = '';
            for(let i=1; i<=4; i++) {
                const date = new Date(d.daily.time[i]);
                const day = i === 1 ? 'Tomorrow' : date.toLocaleDateString('en-US', {weekday:'long'});
...
.forEach(card=>{
    ...
    const openBtn =
        card.querySelector('.folder-expand-btn');

    const panel =
        card.querySelector('.folder-expand-panel');

    const closeBtn =
        card.querySelector('.folder-close-btn');
    ...
    openBtn.addEventListener('click',()=>{
        ...
        /* close others */
        document
        .querySelectorAll('.folder-expand-panel')
        .forEach(p=>{
